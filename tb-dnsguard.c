...
static __always_inline E_DG_Verdict dg_tb_consume_or_refill(u64 elapsed, s64* edt, s64 capacity, s64 packet_cost) {
    ...
    if (usage < 0) {
        ...
        return DG_PASS;
    } else if (usage > (capacity - packet_cost)) {
...
bool dg_consume(void* bucket, u64 now_ns) {
    Bucket* b = bucket;
    E_DG_Verdict verdict = dg_tb_consume_or_refill(now_ns, &b->edt, b->cap, b->packet_cost);
