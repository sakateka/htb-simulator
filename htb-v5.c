...
static __always_inline E_DG_Verdict dg_tb_consume_or_refill(Bucket* tb, u64 elapsed, bool force_take) {
    ...
    if (!force_take && usage > (tb->cap - tb->packet_cost)) {
...
static __always_inline E_DG_Verdict dg_htb_consume_or_refill(Bucket* tb, u64 elapsed) {
    E_DG_Verdict my_v = dg_tb_consume_or_refill(tb, elapsed, false);
    if (my_v == DG_PASS) {
        dg_tb_consume_or_refill(global, elapsed, true);
        dg_tb_consume_or_refill(tb->bb, elapsed, true);
        return DG_PASS;
    }
    E_DG_Verdict burst_v = dg_tb_consume_or_refill(tb->bb, elapsed, false);
    if (burst_v == DG_PASS) {
